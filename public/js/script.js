...
function renderProductDetails() {
    ...
    try {
        ...
        if (product.type === 'mattresses' && product.sizes?.length > 0) {
            ...
            // Цей блок залишиться без змін, якщо не потрібно
        } else {
            if (isOnSale) {
                const saleSpan = document.createElement('span');
                saleSpan.className = 'sale-price';
...
async function addToCartWithColor(productId) {
    ...
    if (typeof product.id !== 'number' || !product.id) {
    ...
    console.log('Додавання до кошика, продукт:', {
        id: product.id, // Змінено з _id на id
        name: product.name,
        slug: product.slug,
        price: product.price,
    ...
    const cartItem = {
        id: product.id, // Змінено з product._id на product.id
        name: product.name,
        quantity: quantity,
        price: price,
...
async function addGroupToCart(productId) {
    ...
    checkboxes.forEach(cb => {
        ...
        if (p) {
            const price = p.salePrice && new Date(p.saleEnd) > new Date() ? p.salePrice : p.price || 0;
            const cartItem = {
                id: p.id, // Змінено з Number(p.id) на p.id
                name: p.name,
                price,
                quantity: 1,
...
async function openProduct(slugOrId) {
    ...
    let product = null;
    if (typeof slugOrId === 'string' && slugOrId.includes('-')) { // Припускаємо, що _id містить дефіси
...
async function submitOrder() {
    ...
    const orderData = {
        ...
        customer,
        items: cart.map(item => ({
            id: item.id, // Використовуємо числове id
            name: item.name,
            quantity: item.quantity,
            price: item.price,
    ...
    if (orderData.items) {
        orderData.items = orderData.items.filter(item => {
            const isValid = item && typeof item.id === 'number' && item.id; // Змінено перевірку на number
            if (!isValid) {
