...
app.put('/api/categories/:id', authenticateToken, csrfProtection, async (req, res) => {
    ...
    try {
        ...
        if (categoryData.name && categoryData.name !== category.name) {
            const existingCategory = await Category.findOne({ name: categoryData.name, _id: { $ne: category._id } }).session(session);
            if (existingCategory) {
        ...
        if (categoryData.slug && categoryData.slug !== category.slug) {
            const existingCategory = await Category.findOne({ slug: categoryData.slug, _id: { $ne: category._id } }).session(session);
            if (existingCategory) {
...
app.put('/api/categories/:categoryId/subcategories/:subcategoryId', authenticateToken, csrfProtection, async (req, res) => {
    ...
    try {
        ...
        if (subcategoryData.name !== oldSubcategory.name) {
            await Product.updateMany(
                { subcategory: oldSubcategory.name },
        ...
        const changesApplied = {
            ...
            photo: subcategoryData.photo !== oldSubcategory.photo,
            visible: subcategoryData.visible !== oldSubcategory.visible,
            order: subcategoryData.order !== oldSubcategory.order
        };
