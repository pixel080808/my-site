...
app.post('/api/cleanup-images', authenticateToken, csrfProtection, async (req, res) => {
    try {
        if (req.user.role !== 'admin') {
            logger.info('Спроба доступу до /api/cleanup-images без прав адміністратора:', req.user);
            return res.status(403).json({ error: 'Доступ заборонено. Потрібні права адміністратора.' });
        }

        // Збираємо всі URL зображень, які використовуються
        ...
        products.forEach(product => {
            if (product.photos && product.photos.length > 0) {
                product.photos.forEach(photo => usedImageUrls.add(photo));
            }
            if (product.colors && product.colors.length > 0) {
                product.colors.forEach(color => {
                    if (color.photo) usedImageUrls.add(color.photo);
                });
            }
        });
        ...
        slides.forEach(slide => {
            if (slide.photo) usedImageUrls.add(slide.photo);
        });

        // Категорії та підкатегорії
        const categories = await Category.find();
        categories.forEach(category => {
            if (category.photo) usedImageUrls.add(category.photo);
            if (category.subcategories && category.subcategories.length > 0) {
                category.subcategories.forEach(sub => {
                    if (sub.photo) usedImageUrls.add(sub.photo);
                });
            }
        });

        // Налаштування (logo, favicon)
        const settings = await Settings.findOne();
        if (settings) {
            if (settings.logo) usedImageUrls.add(settings.logo);
            if (settings.favicon) usedImageUrls.add(settings.favicon);
        }

        // Отримуємо всі зображення з Cloudinary (в межах папки 'products')
        const cloudinaryImages = await cloudinary.api.resources({
            resource_type: 'image',
            prefix: 'products',
            max_results: 500 // Налаштуйте за потребою
        });
        ...
        for (const resource of cloudinaryImages.resources) {
            ...
            if (!usedImageUrls.has(imageUrl)) {
                imagesToDelete.push(resource.public_id);
            }
        }

        // Видаляємо невикористані зображення
        let deletedCount = 0;
        for (const publicId of imagesToDelete) {
            try {
                await cloudinary.uploader.destroy(publicId);
                logger.info(`Видалено невикористане зображення з Cloudinary: ${publicId}`);
                deletedCount++;
            } catch (err) {
                logger.error(`Не вдалося видалити зображення з Cloudinary: ${publicId}`, err);
            }
        }

        logger.info(`Адміністратор ${req.user.username} викликав очищення зображень. Кількість видалених: ${deletedCount}`);
        res.status(200).json({ message: `Невикористані зображення видалено. Кількість: ${deletedCount}` });
    } catch (err) {
        logger.error('Помилка при очищенні невикористаних зображень:', err);
        res.status(500).json({ error: 'Помилка при очищенні зображень', details: err.message });
    }
});
